Shader "Custom/CookTorrance - GlossyRef -Lightmap"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            inline fixed4 LightingCookTorrance (SurfaceOutput s, fixed3 lightDir, fixed3 viewDir, fixed atten)
            {
                ...
                float3 spec = (fresnel * geo * roughness_beckmann ) / ( NdotV * NdotL + 0.02); // 分母的pi一般被认为是包含到light参数里面去了所以这里不算
				//spec  *= Cspec;
			...
			inline float3 ObjSpaceLightDir()
			{
				_WorldSpaceLightDir.xyz = normalize(_WorldSpaceLightDir.xyz);
				_WorldSpaceLightDir.w = 0;
				return mul(_World2Object, _WorldSpaceLightDir).xyz;
			}
...
#else
				float3 lightDir = mul (rotation, ObjSpaceLightDir());
#endif
